...
function guessingGame() {
  ...
  if (guess === numberToGuess && count < 3) {
    ...
    Success: the number is indeed ${numberToGuess}. You got the number correctly after ${count} attempts.`;
  }
   else {
    // Otherwise print that the game is over
    document.getElementById('result').innerText = `Sorry human, but you couldn't guess the number in less than three times. The number was ${numberToGuess}`;
  }
}
